<template>
	...
	<view class="txt flex1">充值记录</view><text class="iconfont icon-arrR c_999 fs_12"></text>
		...
		<!-- <view class="uni__listview mt_15">
			<view class="item uni__list uni__material" @tap="logoutSys">
				<view class="txt flex1 align_c">退出当前账号</view>
			</view>
		</view> -->
...
<script>
	...
	export default {
		...
		methods: {
			refreshBing() {
				Api.httpResponse("/stm/api/user/showUser/refreshBing", 'POST', {
					id: this.userData.id
				}).then(
					resUser => {
						...
						uni.navigateTo({
							url: '/pages/ucenter/bindWxVideoAccount'
						})
					},
					error => {
						console.log(error);
			...
			GoOutMoneyList() {
				uni.navigateTo({
					url: '/pages/ucenter/outMoneyList'
				})
			},
			ToSetPassword() {
				uni.navigateTo({
					url: '/pages/ucenter/setPassword'
				})
			},
			callKefu() {
				uni.makePhoneCall({
			...
			getMoney() {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							Api.httpResponse("/user/showMoney/withdrawal", 'POST', {
								userId: this.userData.id,
								doMoney: this.userData.balance
							}, 'json').then(
								res => {
									Api.httpResponse("/stm/api/user/showUser/getById", 'get', {
										id: this.userData.id
