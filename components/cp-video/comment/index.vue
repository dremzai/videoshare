					...
					<view class="wrap_commentPanel flexbox flex_col">
						...
						<view class="wrap_cmtFoot">
							<view class="wrap__editorPanel flexbox uni_borT" @tap="handleShowFloatInput">
								<view class="wrap_editor flex1"><input class="editor" placeholder="留下你的精彩评论吧"
										disabled /></view>
								<view class="wrap_editor_btn"><text class="iconfont icon-emoj"></text></view>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="floatInput-body">
				<view class="wrap__editorPanel flexbox">
					<view class="wrap_editor flex1">
						<textarea class="editor" v-model="editorText" show-confirm-bar="false" cursor-spacing="15"
							:selection-start="editorLastCursor" :selection-end="editorLastCursor" auto-height
							@input="bindEditorInput" @focus="bindEditorFocus" @blur="bindEditorBlur"
							placeholder="留下你的精彩评论吧" />
					</view>
					<!-- <view class="wrap_editor_btn" @tap="handleShowEmotion"><text class="iconfont icon-emoj"></text>
 					</view> -->
					<view class="wrap_editor_btn btn_submit uni__btn-primary bg_linear2" @tap="handleSubmit"><text
							class="iconfont icon-send"></text></view>
				</view>
				<!-- <view class="pad10 bg_fff"><label class="radio">
								...
								<swiper-item>
									<view class="face_list">
										<block v-for="(item2,index2) in item.nodes" :key="index2">
											<view class="item" @tap="handleEmotionTaped(item2)">
												<image v-if="item2 == 'del'" class="del" src="/static/emotion_del.png"
													mode="widthFix" /><text v-else class="emoj">{{item2}}</text>
											</view>
										</block>
									</view>
								</swiper-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getList() {
				Api.httpResponse("/video/showVideoComment/list", 'GET', this.listQuery).then(
					res => {
			...
			handleLongPressMenu() {
				...
				uniPop.show({
					skin: 'androidSheet',
					contentStyle: 'font-size:16px;font-weight:700;padding-bottom:5px;',
					btns: [{
						text: '回复',
					}, {
						text: '私信回复',
...
<style scoped>
	...
	.wrap_cmtList .item:last-child:after {
		display: none;
	}

	.wrap_cmtList .item .avator {
		border-radius: 50%;
		margin-right: 30upx;
		height: 30px;
		width: 30px;
	...
	.wrap_cmtList .item .cmtinfo .name {
		color: #999;
		font-size: 24upx;
	}

	.wrap_cmtList .item .cmtinfo .cnt {
		font-size: 28upx;
		display: block;
		margin-top: 10upx;
	}

	.wrap_cmtList .item .cmtinfo .time {
		color: #bbb;
		font-size: 24upx;
		margin-top: 10upx;
	}

	.wrap_cmtList .item .like {
		color: #bbb;
		font-size: 24upx;
		text-align: center;
		padding-left: 30upx;
	...
	.wrap_cmtList .item .like .iconfont {
		color: #bbb;
		display: block;
	}
	...
	.wrap__editorPanel.uni_borT:before {
		background: #bdbdbd;
	}
	...
	.wrap__editorPanel .wrap_editor_btn {
		align-self: flex-end;
		justify-content: center;
		padding: 0 4px;
		height: 28px;
	...
	.wrap__editorPanel .wrap_editor_btn .iconfont {
		color: #999;
		font-size: 25px;
	}

	.wrap__editorPanel .btn_submit {
	...
	.wrap__editorPanel .btn_submit .iconfont {
		color: #999;
		font-size: 25px;
	}
	...
	.wrap_emotion .emotion__cells {
		height: 100%;
		position: relative;
	}

	.emotion__cells .face_list {
		display: flex;
		flex-wrap: wrap;
		margin-right: -15upx;
	...
	.emotion__cells .face_list .item:active {
		background: #ebebeb;
	}

	.emotion__cells .face_list .item .emoj {
		font-size: 50upx;
	}

	.emotion__cells .face_list .item .del {
		height: 60upx;
		width: 60upx;
	}

	.anim-footer {
		animation: anim_footer .3s;
	}
